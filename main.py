...
#Plot number of NaN's per year, which could be related to discrepancy between different ways of
...
#now, for plotting, remove years out of range
 ...
 #   plt.show()
    plt.savefig('./figures/missing_points_per_year.png')
    plt.clf()
...
#We will investigate three ways to replace missing data: interpolation, applying average from that month,
#and interpolating between the nearest data points from that month.
...
#    plt.show()
    plt.savefig('./figures/mean_vs_interpolated_temp.png')
    plt.clf()

#From above plot, see there are parts where mean-filled temp is >25 but interpolated temp < 15;
#Focus on these data to look at discrepancy between interpolation and mean filling of missing data
...
#by looking at list of cities in data_nan2, cities where interpolated temp is much lower than mean temp
#are 4 cities; focus on Delhi, so create data frame of just Delhi data

    delhi=data[(data['City'] == 'Delhi') & (data['year']>1855) & (data['year']<1875)]
...
#         .count()'''
    ...
    agg_rolling = agg_d.loc[(agg_d.index >= 1998) & (agg_d.index < 2013)].rolling(10, min_periods=1).mean()
#    print(agg_rolling)
#    del agg_d
    ...
    # take the rolling average over a 10 year period where year greater than 1744
    # Plot results for all three types of NaN strategies
...
#With 10 year average temperatures, take slopes of ten year periods, then do ten year rolling average to see
# if/how much temperature changes with time

    rdf = agg_rolling #regression data frame; same as agg_rolling, but make new df to avoid
                        #reindexing issues when we're plotting
#    print('Regression \n', rdf)
...
#run through data frame, run regression on each ten year chunk, and save resulting slopes to plot
    for year in range(1850, 2013-10+1):
        rdf2 = rdf.loc[(rdf['year'] >= year) & (rdf['year'] <= year + 9)]
        Y=rdf2['MeanFilledTemperature2']
        X=rdf2['year']
        X=sm.add_constant(X)

        linear_fit=sm.OLS(Y,X).fit()
        intercepts.append(linear_fit.params[0])
        slopes.append(linear_fit.params[1])
        avg_rise.append(mean(slopes))

#        if linear_fit.params[1]>0:
#            rising+=1
#        elif linear_fit.params[1]<0:
#            falling+=1

#        print(slope)
#        print(intercept)
#        plt.plot(agg_rolling.index,Y, 'bo')
 #   print(X)
#    plt.plot(agg_rolling.index, slope*agg_rolling.index+intercept, 'ro')
 #   print(X)

#    plt.plot(range(1850, 2013-10+1), slopes)
#    plt.show()

#    plt.plot(range(1850, 2013-10+1), avg_rise)
#    plt.show()

 #   plt.bar(['rising', 'falling'], [rising, falling], )
 #   plt.show()

#rolling average on slopes
    print(len(slopes))
    print(len(range(1850, 2013-10+1)))
    df_slopes=pd.DataFrame({'slopes': slopes})
    print(df_slopes)
    df_slopes['year']=range(1850, 2013-10+1)
    print(df_slopes)
    df_slopes['rolling']= df_slopes['slopes'].rolling(10, min_periods=1).mean()
    print(df_slopes)

    df_slopes.plot(x='year', y='rolling', legend=False)
    plt.axhline(0, color="gray")
    plt.ylabel('10 year average temp change ($^\circ$C/year)')
    plt.savefig('./figures/temp_change_rolling_average.png')
    plt.show()

#Troubleshooting
#    rdf2=rdf.loc[(rdf['year'] >= 1998) & (rdf['year'] < 2008)]
#    print('rdf2\n', rdf2)
#    Y = rdf2['MeanFilledTemperature2']
#    X = rdf2.index
#    print(X)
...
#    print(slope)
#    print(intercept)
#    plt.plot(rdf2.index, Y, 'bo')
#    #   print(X)
#    plt.plot(rdf2.index, slope*rdf2.index+intercept, 'ro')
#    plt.show()
